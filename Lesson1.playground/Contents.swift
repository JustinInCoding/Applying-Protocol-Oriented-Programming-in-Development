...
class FamilyCar: Vehicle {
    ...
    func move(direction: Direction, duration: TimeInterval, speed: Double) {
			let travellingSpeed: Double
			if speed < 0 {
				travellingSpeed = 0
			} else if speed > maxSpeed {
				travellingSpeed = maxSpeed
			} else {
				travellingSpeed = speed
			}
			let distance = travellingSpeed * duration
